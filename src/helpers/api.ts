...
const API_KEY ='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';
const headers = {
  headers: {
...
export function fetchGitHubSearch (query: string): Promise<Search> {
  return fetch('https://api.github.com/search/users?q=' + query, headers)
    .then(response => response.json())
};

export function fetchGitHubUsers (): Promise<User[]> {
  return fetch('https://api.github.com/users', headers)
    .then(response => response.json())
};
...
export function fetchGitHubRepos (url: string): Promise<Repo[]> {
  return fetch(url + "?per_page=1000", headers)
    .then(response => response.json())
};
...
export const queries = async (users: User[]) => {
  ...
  const organizations = await Promise.all(users.map((user) => {
      return fetchGitHubOrganizations(user.organizations_url)
  }));

  const repos = await Promise.all(users.map((user) => {
...
//   const repos = await Promise.all(users.map((user) => {
//       return fetchGitHubRepos(user.repos_url)
//   }));
