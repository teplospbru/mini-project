...
export const UserProfilePage: FC = () => {
  ...
  useEffect(() => {
    if (id) {
      fetchGitHubSingleUser(id).then((response) => {
        setUser(response);
      });
    }
  }, []);
  ...
  useEffect(() => {
    if (user?.repos_url) {
      fetchGitHubRepos(user.repos_url).then((response) => setRepos(response));
    }
  }, [user]);
                    ...
                    <section className="repository-list__item" key={repo.id}>
                      {repo.name ? (
                        <h3 className="repository-list__item-title">
                          <a href={repo.svn_url} className="link">
                            {repo.name}
                          </a>
                        </h3>
                      ) : null}
